<template>
  <v-app>
    <v-toolbar density="compact"
      ...
      <v-toolbar-title v-if="!sidebar" class="grow"> starkify </v-toolbar-title>
      <v-toolbar-title v-if="sidebar" class="grow" style="-webkit-app-region: no-drag"> starkify </v-toolbar-title>
    ...
    <v-data-table v-if="table" :headers="headers" :items="players" :items-per-page="-1" class="datatable elevation-0"
      ...
      <template v-slot:item="{ item }">
        <tr>
          <td>
            <span v-bind="props" v-if="headers.some((h) => h.title === 'E')">
              <span v-html="item.columns.encounters"></span>
            </span>
          </td>
          <td>
            ...
            <v-tooltip v-for="tag in item.columns.tags" location="bottom">
              <template v-slot:activator="{ props }">
                <v-chip v-bind="props" class="ma-1" size="x-small" :color="tag.color" :prepend-icon="tag.prependIcon"
                  :append-icon="tag.appendIcon">{{ tag.text }}</v-chip>
              </template>
          ...
          <td v-if="headers.some((h) => h.title === 'Gaps')"><span v-html="item.columns.gaps"></span></td>
          ...
          <td>
            <v-menu>
              ...
              <v-list>
                <v-list-item>
                  <v-btn variant="flat" prepend-icon="mdi-chart-timeline-variant-shimmer" router-link to="/statistics"
...
import { playerFriendsDictionaries, queueDictionaries, legacyQueuesDictionaries, playerProfileDictionaries } from "./misc/overlay";
...
function pingColorParser(ping) {
  if (ping < 100) return "§a" + ping;        // Green for ping under 100
  if (ping < 160) return "§2" + ping; 
  if (ping <200) return "§e" + ping       // Yellow for ping under 200
  return "§c" + ping;                        // Red for ping 200 and above
}
...
const excludedPlayersString = dataStore.get('excludedPlayers');
const excludedPlayers = excludedPlayersString.split(',').map(player => player.trim());
    ...
    if (Player?.cause) {
      if (Player.cause.toLowerCase() === "invalid uuid or username") {
        ...
        Players.push({
          ...
          tags: [{ text: "NICKED ", tooltip: "Some error occured while fetching!", color: "red-lighten-1" }],
        });
      }
    } else {
     ...
     let isFriend = false;
let friendTagName = ''; // Initialize the variable

for (const [friendName, friendData] of Object.entries(playerFriendsDictionaries)) {
  if ((friendData.friends && Object.values(friendData.friends).some(friend => friend.name === Player.username)) || Player.username in friendData.friends) {        
    isFriend = true;
        friendTagName = friendName; // Assign the correct variable name

        // If the friend does not have an entry in playerFriendsDictionaries, create one
        if (!playerFriendsDictionaries[Player.username]) {
            playerFriendsDictionaries[Player.username] = {
                ign: Player.username,
                friends: { [friendName]: { /* additional friend data if needed */ } }
            };
          
        } else {
            // If the friend already has an entry but does not have this player in their friends list, add them
            if (!playerFriendsDictionaries[Player.username].friends[friendName]) {
                playerFriendsDictionaries[Player.username].friends[friendName] = { /* additional friend data if needed */ };
            }
        }
        break;
    }
      ...
      if (Player.exists === false) {
        ...
        for (const [playerName, playerInfo] of Object.entries(playerProfileDictionaries)) {
          ...
          if (playerInfo.members && playerInfo.members[normalizedPlayerUUID] && playerInfo.members[normalizedPlayerUUID].first_join) {
            const firstJoinTimestamp = playerInfo.members[normalizedPlayerUUID].first_join;
            const oneWeekInMilliseconds = 7 * 24 * 60 * 60 * 1000; // One week in milliseconds

            if (Date.now() - firstJoinTimestamp > oneWeekInMilliseconds) {
              isMember = true;
              memberOfPlayerName = playerName; // The name of the player whose members list includes the target player
              break;
            }
          }
        }
...
// Assuming Player.exists and Player.UUID are defined earlier in your code
if (Player.exists === false) {
    ...
    for (const playerName in legacyQueuesDictionaries) {
        if (legacyQueuesDictionaries.hasOwnProperty(playerName)) {
            ...
            const uuids = legacyQueuesDictionaries[playerName];

            // Check if the player's UUID appears in the uuids array
            if (uuids.includes(normalizedPlayerUUID)) {
                legacyQueued = true;
                Lqueuedmember = playerName; // Assign the correct variable name
                LqueueCount++; // Increment the count for each match found
                // Removed the break statement to count all matches
            }
        }
    }
}
      ...
      let queuedMembers = [];
      let q = ''
      // Replace dashes from the player's UUID
      if (Player.exists === false) {
        console.log("skipping invalid player")
      }
      else {
        ...
        for (const playerName in queueDictionaries) {
          if (queueDictionaries.hasOwnProperty(playerName)) {
            ...
            if (count > 0) {
              Queued = true; // Set Queued to true if the UUID is found
              queueCount += count;

              // Make a request to the Mojang API to get the player's ID and name
              axios.get(`https://api.mojang.com/user/profile/${playerName}`)
                .then(response => {
                  // Handle the API response here
                  const playerInfo = response.data; // Assuming the response is an array with the most recent name as the first element
                  q = {
        ...
        if (isFriend) {
        // Change the tag for friends
        tags.push({ text: "C", tooltip: `F ${friendTagName}`, color: "cyan-accent-2" });
        } else if (isMember) {
          tags.push({ text: "C", tooltip: `C ${memberOfPlayerName}`, color: "cyan-accent-2" });
        } else if (legacyQueued) {
          tags.push({ text: "C", tooltip: `LQ ${Lqueuedmember}-${LqueueCount} `, color: "cyan-accent-2" });
        } else if (Queued) {
          tags.push({ text: "C", tooltip: `Q-${queueCount}`, color: "cyan-accent-2" });
      ...
      Players.push({
        ...
        Ping: mcColorParser(pingColorParser(Player.ping)), // Replace with the actual property name for ping data
...
const updateHeaders = () => {
  ...
  if (selectedHeaders.includes("Wins")) headers.value.push({ title: "Gaps", align: "center", key: "gaps", width: "10%" });
  if (selectedHeaders.includes("Gaps")) headers.value.push({ title: "Gaps", align: "center", key: "gaps", width: "10%" });
