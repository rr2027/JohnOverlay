...
//   const playerData = await axios.get(`youtube.com/${playerName}`, { headers: { "X-API-Key": dataStore.get("pixelicKey"), "cache-control": "no-cache" } }).catch((error) => console.error("Invalid Hypixel Player"));
...
//   axios.get(`youtube.com/${playerName}`, { headers: { "X-API-Key": dataStore.get("pixelicKey") } }).then((historical) => {
