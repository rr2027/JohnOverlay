    ...
    <v-container>
        ...
        <v-col>
          <v-card title="General">
            <div class="ml-4 mr-4 mt-4">
              <v-slider thumb-label :min="0" :max="100" :step="1" color="secondary" label="Opacity"
                prepend-icon="mdi-opacity" v-model="opacity" @update:model-value="setOpacity"></v-slider>
              <v-list density="compact">
                <v-list-item>
                  ...
                  <template v-slot:append>
                    <v-switch color="secondary" v-model="hideIngame" @change="toggleHideIngame"
      ...
      <v-row>
        <v-col>
          <v-card title="Theme">
            <div class="ml-4 mr-4 mt-4">
              <v-select label="Theme" variant="outlined" :items="themes" prepend-inner-icon="mdi-palette-outline"
                v-model="currentTheme" return-object @update:modelValue="setTheme"></v-select>
              ...
              <div v-if="theme.global.name.value === 'custom'">
                <v-select label="Component" variant="outlined" :items="themeComponents"
                  prepend-inner-icon="mdi-palette-outline" v-model="previewedComponent" return-object
                  @update:modelValue="setThemeComponent"></v-select>
              </div>
              <div class="d-flex justify-center mb-4">
                <v-color-picker v-if="theme.global.name.value === 'custom'" return-object
                  @update:modelValue="setThemeComponentColor" hide-inputs mode="hex"></v-color-picker>
              </div>
            </div>
          </v-card>
        </v-col>
